...
export default function Dashboard() {
  ...
  const stats = [
    {
      name: "Total Leads",
      value: isSignedIn ? (leads?.length ?? 0) : "Login to view",
      icon: (
    ...
    },
    {
      ...
      value: isSignedIn ? (qualifiedLeads?.length ?? 0) : "Login to view",
      icon: (
    ...
    },
    {
      name: "Average Score",
      value: isSignedIn 
        ? (leads?.length ? (leads.reduce((acc, lead) => acc + (lead.leadScore ?? 0), 0) / leads.length).toFixed(1) : "0")
        : "Login to view",
      icon: (
    ...
    },
    {
      name: "Last Scan",
      value: isSignedIn 
        ? (leads?.length ? new Date(Math.max(...leads.map(lead => new Date(lead.lastScanned ?? 0).getTime()))).toLocaleDateString() : "Never")
        : "Login to view",
      icon: (
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col justify-between gap-4 rounded-lg bg-gray-800 p-6 md:flex-row md:items-center">
            <div>
              <h1 className="text-2xl font-semibold text-white">
                {isSignedIn ? `Welcome back, ${user?.firstName || 'User'}!` : 'Welcome to SuperCells'}
              </h1>
              <p className="mt-1 text-gray-300">
                {isSignedIn 
                  ? "Here's what's happening with your lead generation today."
                  : "Login to access your AI-powered lead generation dashboard."}
              </p>
            </div>
            {isSignedIn ? (
              <button 
                onClick={() => window.location.href = "/dashboard/leads/new"}
                className="flex items-center justify-center gap-2 self-start rounded-lg bg-gradient-to-r from-orange-300 via-pink-500 to-purple-400 px-4 py-2 font-medium text-white shadow-lg transition-all hover:from-orange-400 hover:via-pink-600 hover:to-purple-500 md:self-center"
              >
          ...
          <div className="rounded-lg bg-gray-800 p-6 shadow">
            ...
            {isSignedIn ? (
              ...
              <div className="flex flex-col items-center justify-center rounded-lg bg-gray-700/50 py-16 text-center">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="mb-4 h-16 w-16 text-gray-500">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                </svg>
                <h3 className="mb-2 text-xl font-medium text-white">Login to View Your Leads</h3>
                <p className="mb-6 max-w-md text-gray-400">Sign in to access your AI-powered lead management system and start discovering high-potential leads automatically.</p>
                <SignInButton mode="modal">
                  <button className="rounded-lg bg-gradient-to-r from-orange-300 via-pink-500 to-purple-400 px-6 py-3 font-medium text-white shadow-lg transition-all hover:from-orange-400 hover:via-pink-600 hover:to-purple-500">
                    Login Now
                  </button>
                </SignInButton>
              </div>
